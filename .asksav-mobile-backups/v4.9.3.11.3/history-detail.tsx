// AskSAV Mobile v4.9.3.11.2 - Saved Analysis persisted appearance repair
...
import { effectiveScheme,loadAskSAVAppearance,type AskSAVAppearance } from "../lib/appearance";
...
export default function HistoryDetail(){
 ...
 const [appearance,setAppearance]=useState<AskSAVAppearance>("system");
 const [x,setX]=useState<any>();

 const refreshAppearance=useCallback(()=>{
   void loadAskSAVAppearance().then(setAppearance);
 },[]);
 useEffect(()=>{refreshAppearance();},[refreshAppearance]);
 useFocusEffect(useCallback(()=>{refreshAppearance();},[refreshAppearance]));
 useEffect(()=>{AsyncStorage.getItem("asksav.mobile.history.v1").then(r=>setX((r?JSON.parse(r):[]).find((v:any)=>v.id===id)||null))},[id]);

 const dark=effectiveScheme(appearance,systemScheme)==="dark";
