...
export default function HomeScreen() {
  ...
  async function takePhoto() {
    ...
    if (!result.canceled && result.assets[0]?.uri) { setImageUri(result.assets[0].uri); setAnalysis(null); setMarketData(null); setMarketError(null); }
  }
  ...
  async function loadMarketIntelligence() {
    ...
    try {
      ...
      const base=(process.env.EXPO_PUBLIC_ASKSAV_API_BASE_URL || "https://asksav.ai").replace(/\/$/,"");
      const response=await fetch(base+"/api/v20/market-intelligence",{
        method:"POST",
        ...
        body:JSON.stringify({
          ...
          usage:analysis?._asksav?.usage ?? analysis?.usage ?? {}
        })
      });
      const payload=await response.json().catch(()=>({}));
      if(!response.ok) throw new Error(payload?.error || payload?.message || ("Market Intelligence failed ("+response.status+")"));
      setMarketData(payload?.marketIntelligence ?? payload?.market ?? payload);
    } catch(e:any) {
        ...
        <ScrollView contentContainerStyle={s.previewPage}>
          ...
          {!analysis ? <TouchableOpacity style={s.primary} onPress={usePhoto} disabled={analysing}>
            ...
            {marketData ? <View style={s.marketResultCard}>
              ...
              {first(marketData?.evidence_summary) ? <Text style={s.marketEvidence}>{first(marketData?.evidence_summary)}</Text> : null}
...
const s=StyleSheet.create({
 ...
 marketEvidence:{color:"#4f6f7b",fontSize:14,lineHeight:21,marginTop:12},
