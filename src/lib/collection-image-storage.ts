...
export async function uploadCollectionImage(itemId:string,localUri:string):Promise<CloudCollectionImage>{
  ...
  // Upload the local JPEG as native binary content. This avoids both the
  // Firebase JS SDK Blob/ArrayBuffer path and the Base64-as-JPEG corruption
  // that occurred when a Base64 string was used as the REST request body.
  ...
  const responseText=uploadResponse.body||"";
  let metadata:any={};
  try{
