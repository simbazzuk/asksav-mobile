...
type Item={id:string;savedAt:string;name?:string;category?:string;condition?:string;confidence?:string;imageUri?:string;cloudImageUrl?:string;cloudImagePath?:string;analysis?:any;market?:any;marketUpdatedAt?:string;valueHistory?:ValuePoint[]};
...
export default function CollectionScreen(){
 ...
 useFocusEffect(
  useCallback(()=>{
   requestAnimationFrame(()=>collectionScrollRef.current?.scrollTo({y:0,animated:false}));
   return undefined;
  },[])
 );

 return <SafeAreaView style={s.safe}><ScrollView ref={collectionScrollRef} contentOffset={{x:0,y:0}} contentContainerStyle={s.page}>
  <AskSAVPageHeader title="My Collection" description="Your saved items, values and market history."/><View style={s.syncRow}><Ionicons name={syncState==="synced"?"cloud-done-outline":syncState==="failed"||syncState==="offline"?"cloud-offline-outline":syncState==="syncing"?"sync-outline":"phone-portrait-outline"} size={13} color={DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"})}/><Text style={s.syncText}>{syncState==="synced"?"Synced to your AskSAV account":syncState==="failed"?"Cloud sync unavailable - saved on this device":syncState==="offline"?"Cloud sync unavailable - saved on this device":syncState==="syncing"?"Syncing with AskSAV Cloud...":"On this device - sign in to sync"}</Text></View>

  <View style={s.collectionTools}>
   <View style={s.searchBox}><Ionicons name="search-outline" size={18} color={DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"})}/><TextInput style={s.searchInput} value={collectionSearch} onChangeText={setCollectionSearch} placeholder="Search your Collection" placeholderTextColor={DynamicColorIOS({light:"#789097",dark:"#e0eeee"})} returnKeyType="search"/>{collectionSearch?<TouchableOpacity onPress={()=>setCollectionSearch("")}><Ionicons name="close-circle" size={18} color={DynamicColorIOS({light:"#789097",dark:"#e0eeee"})}/></TouchableOpacity>:null}</View>
  ...
  {items.length===0?<View style={s.empty}>
   ...
   return <TouchableOpacity key={item.id} style={s.card} activeOpacity={0.82} onPress={()=>router.push({pathname:"/collection-detail",params:{id:item.id}})}>
    ...
    <View style={s.cardTop}>
     {(item.cloudImageUrl||item.imageUri)?<Image source={{uri:item.cloudImageUrl||item.imageUri}} style={s.image}/>:<View style={[s.image,s.placeholder]}><Ionicons name="image-outline" size={28} color={DynamicColorIOS({light:"#789097",dark:"#e0eeee"})}/></View>}
     <View style={s.body}>
  ...
  <View style={s.disclaimer}><Ionicons name="information-circle-outline" size={16} color={DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"})}/><Text style={s.disclaimerText}>Collection values are indicative Market Intelligence, not formal valuations or guaranteed sale prices.</Text></View>
...
const s=StyleSheet.create({
 safe:{flex:1,backgroundColor:DynamicColorIOS({light:"#f4fbf9",dark:"#07191d"})},page:{paddingHorizontal:20,paddingTop:4,paddingBottom:110},
 brandRow:{flexDirection:"row",alignItems:"center",gap:11},brandLogo:{width:58,height:58,borderRadius:16},brandCopy:{flex:1},syncRow:{flexDirection:"row",alignItems:"center",gap:4,marginTop:4},syncText:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:9,fontWeight:"700"},logo:{width:48,height:48,borderRadius:15,backgroundColor:"#13b8aa",alignItems:"center",justifyContent:"center"},logoText:{color:DynamicColorIOS({light:"#fff",dark:"#10282d"}),fontWeight:"900",fontSize:16},brand:{color:"#082f49",fontSize:24,fontWeight:"900"},brandSub:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:11},
 collectionTools:{marginTop:20,marginBottom:4},searchBox:{minHeight:48,borderRadius:15,borderWidth:1,borderColor:DynamicColorIOS({light:"#cfe8e2",dark:"#24464c"}),backgroundColor:DynamicColorIOS({light:"#fff",dark:"#10282d"}),paddingHorizontal:13,flexDirection:"row",alignItems:"center",gap:8},searchInput:{flex:1,color:DynamicColorIOS({light:"#173f51",dark:"#f5fffd"}),fontSize:14,paddingVertical:10},filterRow:{gap:7,paddingVertical:10},filterChip:{borderRadius:99,borderWidth:1,borderColor:DynamicColorIOS({light:"#cfe8e2",dark:"#24464c"}),backgroundColor:DynamicColorIOS({light:"#fff",dark:"#10282d"}),paddingHorizontal:12,paddingVertical:7},filterChipActive:{backgroundColor:"#087f72",borderColor:"#087f72"},filterChipText:{color:DynamicColorIOS({light:"#557681",dark:"#d6e7e8"}),fontSize:11,fontWeight:"800"},filterChipTextActive:{color:DynamicColorIOS({light:"#fff",dark:"#10282d"})},sortRow:{flexDirection:"row",alignItems:"center",gap:8},sortLabel:{color:DynamicColorIOS({light:"#789097",dark:"#e0eeee"}),fontSize:9,fontWeight:"900",letterSpacing:.7},sortChoices:{gap:6},sortChip:{borderRadius:10,backgroundColor:DynamicColorIOS({light:"#eef6f4",dark:"#123137"}),paddingHorizontal:9,paddingVertical:6},sortChipActive:{backgroundColor:"#d8f1eb"},sortChipText:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:10,fontWeight:"800"},sortChipTextActive:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"})},resultCount:{color:DynamicColorIOS({light:"#789097",dark:"#e0eeee"}),fontSize:10,fontWeight:"700",marginTop:9},clearFilters:{marginTop:14,paddingHorizontal:15,paddingVertical:10,borderRadius:12,backgroundColor:DynamicColorIOS({light:"#e8f7f4",dark:"#17383d"})},clearFiltersText:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"}),fontSize:11,fontWeight:"900"},
 valueDashboard:{backgroundColor:DynamicColorIOS({light:"#e8f7f4",dark:"#17383d"}),borderWidth:1,borderColor:"#b9ddd5",borderRadius:22,padding:19,marginTop:24,marginBottom:14},dashboardEyebrow:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"}),fontSize:10,fontWeight:"900",letterSpacing:.8},dashboardValue:{color:DynamicColorIOS({light:"#062f4f",dark:"#f5fffd"}),fontSize:36,fontWeight:"900",marginTop:4},dashboardNote:{color:DynamicColorIOS({light:"#5f7c86",dark:"#e0eeee"}),fontSize:11,lineHeight:16,marginTop:4},statsRow:{flexDirection:"row",gap:8,marginTop:16},stat:{flex:1,backgroundColor:DynamicColorIOS({light:"rgba(255,255,255,0.72)",dark:"#1d3f45"}),borderRadius:13,paddingVertical:10,alignItems:"center"},statValue:{color:DynamicColorIOS({light:"#062f4f",dark:"#f5fffd"}),fontSize:18,fontWeight:"900"},statLabel:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:10,fontWeight:"800",marginTop:2},
 card:{backgroundColor:DynamicColorIOS({light:"#fff",dark:"#10282d"}),borderWidth:1,borderColor:DynamicColorIOS({light:"#d9ebe7",dark:"#88aeb2"}),borderRadius:18,padding:12,marginBottom:10},cardTop:{flexDirection:"row",gap:12,alignItems:"center"},image:{width:76,height:76,borderRadius:13,backgroundColor:DynamicColorIOS({light:"#eef6f4",dark:"#1d3f45"})},placeholder:{alignItems:"center",justifyContent:"center"},body:{flex:1},name:{color:DynamicColorIOS({light:"#062f4f",dark:"#f5fffd"}),fontSize:15,fontWeight:"900"},category:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:11,marginTop:3},meta:{flexDirection:"row",alignItems:"center",gap:6,marginTop:7,flexWrap:"wrap"},pill:{backgroundColor:DynamicColorIOS({light:"#e8f7f4",dark:"#17383d"}),borderRadius:99,paddingHorizontal:8,paddingVertical:4},pillText:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"}),fontSize:10,fontWeight:"800"},statusPill:{borderRadius:99,paddingHorizontal:7,paddingVertical:4},statusValued:{backgroundColor:DynamicColorIOS({light:"#e8f7f4",dark:"#17383d"})},statusMissing:{backgroundColor:DynamicColorIOS({light:"#fff5e8",dark:"#4a351d"})},statusText:{fontSize:9,fontWeight:"900"},statusTextValued:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"})},statusTextMissing:{color:DynamicColorIOS({light:"#9a6418",dark:"#ffd28a"})},remove:{padding:8},
 valueRow:{borderTopWidth:1,borderTopColor:DynamicColorIOS({light:"#edf4f2",dark:"#24464c"}),marginTop:11,paddingTop:11,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:10},valueLabel:{color:DynamicColorIOS({light:"#789097",dark:"#e0eeee"}),fontSize:9,fontWeight:"900",letterSpacing:.6},value:{color:DynamicColorIOS({light:"#062f4f",dark:"#f5fffd"}),fontSize:22,fontWeight:"900",marginTop:2},noValue:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:14,fontWeight:"800",marginTop:3},date:{color:DynamicColorIOS({light:"#789097",dark:"#e0eeee"}),fontSize:9,marginTop:3},refreshButton:{minHeight:38,borderRadius:12,borderWidth:1,borderColor:"#b9ddd5",backgroundColor:DynamicColorIOS({light:"#f4fbf9",dark:"#07191d"}),paddingHorizontal:11,flexDirection:"row",gap:6,alignItems:"center",justifyContent:"center"},refreshText:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"}),fontSize:11,fontWeight:"900"},
 empty:{backgroundColor:DynamicColorIOS({light:"#fff",dark:"#10282d"}),borderWidth:1,borderColor:DynamicColorIOS({light:"#d9ebe7",dark:"#88aeb2"}),borderRadius:24,padding:24,alignItems:"center",marginTop:8},emptyIcon:{width:68,height:68,borderRadius:34,backgroundColor:DynamicColorIOS({light:"#e8f7f4",dark:"#17383d"}),alignItems:"center",justifyContent:"center"},emptyTitle:{color:DynamicColorIOS({light:"#062f4f",dark:"#f5fffd"}),fontSize:21,fontWeight:"900",marginTop:15},emptyCopy:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:13,lineHeight:20,textAlign:"center",marginTop:6},primary:{minHeight:52,borderRadius:15,backgroundColor:"#0aaf86",flexDirection:"row",gap:8,alignItems:"center",justifyContent:"center",paddingHorizontal:22,marginTop:18},primaryText:{color:DynamicColorIOS({light:"#fff",dark:"#10282d"}),fontSize:14,fontWeight:"900"},
 historyBox:{borderTopWidth:1,borderTopColor:DynamicColorIOS({light:"#edf4f2",dark:"#24464c"}),marginTop:11,paddingTop:11},historyHeader:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:7},historyEyebrow:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"}),fontSize:9,fontWeight:"900",letterSpacing:.7},historyTitle:{color:DynamicColorIOS({light:"#062f4f",dark:"#f5fffd"}),fontSize:12,fontWeight:"800",marginTop:2},changePill:{borderRadius:99,paddingHorizontal:8,paddingVertical:5},changeUp:{backgroundColor:DynamicColorIOS({light:"#e8f7f4",dark:"#17383d"})},changeDown:{backgroundColor:"#fff1ed"},changeFlat:{backgroundColor:"#f0f4f4"},changeText:{color:DynamicColorIOS({light:"#315d63",dark:"#d6e7e8"}),fontSize:9,fontWeight:"900"},historyRow:{minHeight:40,flexDirection:"row",alignItems:"stretch"},timeline:{width:18,alignItems:"center"},dot:{width:8,height:8,borderRadius:4,backgroundColor:"#a8c7c1",marginTop:7},dotLatest:{backgroundColor:"#0a9b80"},line:{width:1,flex:1,backgroundColor:"#d5e8e4",marginVertical:2},historyBody:{flex:1,paddingBottom:8},historyMoney:{color:DynamicColorIOS({light:"#173f51",dark:"#f5fffd"}),fontSize:13,fontWeight:"900"},historyDate:{color:DynamicColorIOS({light:"#789097",dark:"#e0eeee"}),fontSize:9,marginTop:1},latest:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"}),fontSize:8,fontWeight:"900",letterSpacing:.5,marginTop:4},historyHint:{color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:10,lineHeight:15,backgroundColor:DynamicColorIOS({light:"#f4fbf9",dark:"#07191d"}),borderRadius:10,padding:9,marginTop:2},
 openDetail:{borderTopWidth:1,borderTopColor:DynamicColorIOS({light:"#edf4f2",dark:"#24464c"}),marginTop:10,paddingTop:10,flexDirection:"row",alignItems:"center",justifyContent:"flex-end",gap:4},openDetailText:{color:DynamicColorIOS({light:"#087f72",dark:"#42d7bd"}),fontSize:10,fontWeight:"900"},
  disclaimer:{flexDirection:"row",gap:7,backgroundColor:DynamicColorIOS({light:"#f0f6f5",dark:"#10282d"}),borderRadius:13,padding:11,marginTop:5},disclaimerText:{flex:1,color:DynamicColorIOS({light:"#66858d",dark:"#d6e7e8"}),fontSize:10,lineHeight:15},note:{color:DynamicColorIOS({light:"#789097",dark:"#e0eeee"}),fontSize:10,lineHeight:15,textAlign:"center",marginTop:12}
});
